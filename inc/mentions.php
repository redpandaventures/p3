...
class P3_Mentions extends P3_Terms_In_Comments {
	...
	public function __construct() {
		...
		// Hooks
		add_action( 'init',              array( &$this, 'register_mention_tax'            ), 0 );
		...
		parent::init( P3_MENTIONS_TAXONOMY );
	}

	/**
	 * Register P3 mentions taxonomy.
	 */
	function register_mention_tax() {
	...
	function mention_links( $content ) {
		...
		foreach ( $names as $name ) {
			...
			$url = apply_filters( 'p3_mention_url', $url, $name );
