...
P3 is available on WordPress.com: http://wordpress.com/signup/?ref=p3
...or you can download it for WordPress.org: http://wordpress.org/themes/p3

You can also check out a demo of the theme in action: http://p3demo.wordpress.com/
If you need P3 support or want to pitch in, drop a line on the forums: http://wordpress.org/tags/p3

== Further reading ==

Matt Mullenweg: How P3 changed Automattic:
http://ma.tt/2009/05/how-p3-changed-automattic/

Official announcement post on WordPress.com:
http://en.blog.wordpress.com/2009/03/11/p3-the-new-prologue/
...
= 7 August 2013 =
* only implement "p3_hide_threads" theme option when on non-singular views.
...
= 4 May 2013 =
* Call P3's upgrade routine before dispatching AJAX requests.

= 3 May 2013 =
* Typo: stray comma.
* P3 can upgrade itself from blog-side via an AJAX call if it thinks it needs it.
...
= 6 November 2012 =
* Send no-cache headers and 200 response when no results are returned by P3Ajax::get_latest_posts() and P3Ajax::get_latest_comments().
...
* Tweak P3 print styles to remove errant bullets, widen right margin, decrease font size slightly and increase size of post titles.

= 27 September 2012 =
* Add collaboration to style.css tags to match the tags that are on the Showcase Page for P3.

= 12 September 2012 =
* Restore loading of style-iphone.css, remove style-responsive.css

= 11 September 2012 =
* Further responsive styles for P3.
* Add function_exists() call to is_automattician() so we don't break dot org sites using P3 from the themes repo.

= 10 September 2012 =
* Begin initial responsive styling. Simplifying existing styles, hiding unneeded elements, for P3.
* Don't show the iPhone mobile styles when not on iPhone. :)
* Add style-responsive.css to P3, and load this file instead of style-iphone.css for members of Team Cordova to test.
...
= 14 July 2012 =
* P3 Fix margin bug for the post post-format in Chrome nightlies.
...
* Better i18n for strings in p3_get_discussion_links().
...
* Piano Black, Liquorice, P3, The Morning After, Neutra, Clean Home: add generic action-hooks to header and sidebar.
...
= 14 December 2011 =
* remove debug and unused code from r8446.P3: remove debug and unused code from r8446.)
...
= 18 November 2011 =
* Update the core P3 theme to allow instapost to override the default posting interface. If Instapost code cannot be found then the theme will fall back to the original post form.

= 10 November 2011 =
* P3
* Hook into the post_comments_link filter so the reply link also outputs a title attribute (like the other action links)
* Add title attributes to the P3 action links (permalink, edit, etc) so that there are tooltips on link hover.
...
* More P3 style
* More P3 style
* More P3 style fixes for iPhone. (Stupid iPhone simulator is not the same as an actual device)
* More P3 style
* More P3 style
* More P3 style
* More P3 style
* Make sure the post button only shows if the user can the permission to post on a P3.
* More P3 style
* Fix up the iPhone styles for P3 theme so it's actually usable and readable.

= 4 October 2011 =
* improve mentions.php for backcompat with older versions of WP (< 3.1).

= 3 October 2011 =
* P3 should allow you to toggle comment threads on tag archives, props westi.
...
= 20 July 2011 =
* P3, wptouch, Liquorice: Use integer for $content_width value, not string.
...
= 4 July 2011 =
* make sure the special check for logged-out and offline only runs for users P3 thinks are already logged in. See #665.
...
= 8 June 2011 =
* Add p3_found_mentions filter to allow plugins to alter which mentions are attached to a post/comment.
...
= 8 April 2011 =
* Refactor P3 into components, improve mentions and autocomplete, and many minor improvements;
...
= 22 February 2011 =
* Allow hooking into P3 to add new action links.
...
= 11 February 2011 =
* cleanup for p3_excerpted_title.
* add p3_excerpted_title to provide titles with only whole words -
...
= 7 February 2011 =
* supporting p3 code so we can hook in and add items to the post form
...
* i18n for p3 user completion js

= 6 January 2011 =
* initialize arrays to avoid errors

= 5 January 2011 =
* backwards compatible and single install p3_user_suggestion -
...
* fix p3 at name highlighting logic. This should only run when looking at a mentions taxonomy view

= 8 December 2010 =
* add title attribute to show @name mention usernames - closes #549
* Orders tags by popularity and show counts. Have username suggest and tag autocomplete use same JS. Add username suggest and new tag counts for all P3s

= 7 December 2010 =
* revert dropdown change, filter will go here
* linebreak for tags
* remove count
* order tags by popularity for p3s and display count for tag dropdown
...
= 1 December 2010 =
* remove debug
* P3 prep for 1.1.9 release and move changelog.txt to readme.txt
* add isRtl variable and misc cleanup; add P3_INC_URL back in
...
* only load farbtastic and colorpicker on the p3 options page, instead of everywhere on the blog and wp-admin

= 27 September 2010 =
* Update P3 inline editing to support editing tags, post title, and quote fields (closes #184, refs #374)
...
= 10 September 2010 =
* A filter to make it easier to modify P3s special function.
...
* revert r4376 for p3_the_title return parameter

= 9 August 2010 =
* call comment_class correctly
* change back to $echocomments
* fix empty delimiter in p3_the_title -- fixes 462; cleanup and bump version;
...
= 15 June 2010 =
* Use the standard generator and designer links for Neat, Neo-Sapien, Neutra, Ocadia, Ocean Mist, P3, Pool, Pressrow, Prologue -
...
= 3 June 2010 =
* Author Grid widget style updates for Bueno, P3

= 28 May 2010 =
* newComment code cleanup -

= 27 May 2010 =
* p3_title calls don't need default values
...
= 24 May 2010 =
* Add translator credit in P3 changelog
* Add Czech translation of P3
...
= 15 April 2010 =
* changelog update
* adding belorussian translation for p3
...
= 10 December 2009 =
* fixing potential cookie security problem in p3 by attaching to wp_ajax_ action so that can_user() functions actually check for proper cookies
...
* fixes rewrite rule flushing for p3 @mentions
...
* p3 action links, 3rd try. Hope I got it right this time.
* move hook outside of condition so that not logged in users can

= 19 November 2009 =
* use get_permalink instead of guid
* p3, better than [2679] also handles permalinks page
* small fix to action links
* fix notifications and help on p3 rtl
* fixing overflow on code and pre tags
* make no sidebar work for RTL blogs
* p3 - new rtl.css
* p3 rtl updates
* theme name back to P3
* upgrading to p3 1.1 cross fingers

= 23 October 2009 =
* Giving P3 titles some line height so two-line titles aren't squashed
...
= 14 October 2009 =
* cache bust p3 rtl.css
* updated to p3 rtl.css
...
= 22 September 2009 =
* p3 option to allow any wordpress.com member to post

= 20 September 2009 =
* Unbreak P3 for WP.org
...
= 26 August 2009 =
* updating filters for urls in p3
...
* remove p3 debug bug from [2111] which broke everything after the first comment event

= 11 August 2009 =
* trying to solve the p3 cache bug by expiring the json data on the spot
...
* Move ajax from admin-ajax to inside P3 -- the P3Ajax class
...
= 27 May 2009 =
* Bust p3.js cache
* Add Fluid badge count support for P3
...
= 8 April 2009 =
* Attribute escaping and $wpdb->prepare() for P3
...
* updates to p3 to fix bugs
* fixing a mound of bugs for p3
